...
- API calls to backend (default `http://localhost:4000` in dev) via `fetch()` or custom hooks
...
```bash
# Kill existing processes (default ports)
lsof -ti:4000 | xargs kill -9  # API (default)
lsof -ti:3000 | xargs kill -9  # Web (default)
lsof -ti:5432 | xargs kill -9  # PostgreSQL

# Docker Compose uses API port 3001 by default
lsof -ti:3001 | xargs kill -9  # API (Docker)
```

Override ports in `.env.local`:

```bash
API_PORT=4001          # Standalone API
WEB_PORT=3001          # Web frontend
...
- API requires ≥75-84% coverage (see `api/jest.config.js`)
...
- **API**: Minimum 75-84% (branches: 75%, functions: 80%, lines/statements: 84%)
...
**Key CI Pattern**: All workflows use consistent Node.js version (v20+) and pnpm (v8.15.9) defined in package.json for easy updates.
...
**Location**: `mobile/App.tsx` - Main React Native app with Expo

**Key Dependencies**:

- Expo SDK 54+ - Development framework
- React Native 0.74.3 - Cross-platform native components
...
type ShipmentItem = Pick<Shipment, "id" | "destination" | "status">;
```
...
**Environment Variables**: Use `EXPO_PUBLIC_` prefix for public env vars

```bash
EXPO_PUBLIC_API_BASE=https://api.example.com
```

**Secure Storage**: Always use `expo-secure-store` for sensitive data (tokens, keys)
...
**Error Handling**: Mobile networks are unreliable - always implement retry logic
...
- `billing.spec.js` - Payment integration
- `core-features.spec.js` - Main user workflows

### Playwright Configuration

**Key Settings** (`playwright.config.js`):
...
```javascript
// e2e/pages/shipments.page.js
export class ShipmentsPage {
  constructor(page) {
    this.page = page;
    this.createButton = page.locator('[data-testid="create-shipment"]');
    this.shipmentList = page.locator('[data-testid="shipment-list"]');
  }
  ...
  async createShipment(data) {
    ...
    await this.page.fill('[name="origin"]', data.origin);
...
```javascript
// e2e/tests/core-features.spec.js
import { test, expect } from "@playwright/test";

test("complete shipment workflow", async ({ page }) => {
  // 1. Login
  ...
  await expect(page.locator('[data-testid="shipment-list"]')).toContainText(
    "New York",
  );
});
```
...
pnpm e2e e2e/tests/auth.spec.js
...
- Mock external APIs (Stripe, PayPal) in tests

❌ **DON'T**:

- Rely on CSS classes or text content for selectors
- Test every edge case with E2E (use unit tests)
- Run E2E tests in local dev (slow)
- Hardcode URLs (use baseURL config)

## 📊 Monitoring & Observability

### Sentry Integration

**Location**: `api/src/config/sentry.js`
