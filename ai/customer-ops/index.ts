...
export const customerOpsRole: RoleContract = {
  ...
  async checkGuardrails(input: DecisionInput, context: RoleContext): Promise<GuardrailViolation[]> {
    ...
    if (input.action.includes('price') || input.action.includes('rate') || input.action.includes('quote')) {
      violations.push({
        type: 'policy',
        severity: 'high',
    ...
    if ((input.action.includes('refund') || input.action.includes('credit')) && !input.parameters.humanApproval) {
      violations.push({
        type: 'policy',
        severity: 'high',
    ...
    if (JSON.stringify(input).toLowerCase().match(/payment|credit.?card|bank|financial/)) {
      violations.push({
        type: 'data-access',
        severity: 'critical',
  ...
  async calculateConfidence(input: DecisionInput, context: RoleContext): Promise<ConfidenceScore> {
    ...
    return {
      value: 0.92,
      reasoning: 'High confidence based on query type and historical response accuracy',
      factors: {
        dataQuality: 0.93,
        modelCertainty: 0.91,
