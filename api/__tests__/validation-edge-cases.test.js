...
describe("POST /api/users - Edge Cases", () => {
  ...
  describe("Multiple field errors", () => {
    test("returns all validation errors when multiple fields invalid", async () => {
      ...
      expect(res.body.details.map((d) => d.path).includes("email")).toBe(true);
      expect(res.body.details.map((d) => d.path).includes("name")).toBe(true);
      expect(res.body.details.map((d) => d.path).includes("role")).toBe(true);
    });
  });
