 ...
 */
router.get('/live', authenticate, requireScope('metrics:read'), async (req, res, next) => {
  try {
    // Check cache
 ...
 */
router.post('/clear-cache', authenticate, requireScope('admin'), async (req, res) => {
  metricsCache = {
    data: null,
    timestamp: null,
    ttl: 60000,
 ...
 */
router.get('/export', authenticate, requireScope('metrics:export'), async (req, res, next) => {
  try {
    const metrics = await calculateLiveMetrics();
    const mrrHistory = await getMRRHistory(12);
